...
static void update_kb_label_task(lv_timer_t *timer)
{
...
void app_keyboard_load(lv_obj_t *cont)
{
  ...
  kb_timer = lv_timer_create(update_kb_label_task, 500, NULL);
  lv_task_handler();
} 
static void button_event_cb(lv_event_t *e)
{
  ...
  if (code == LV_EVENT_CLICKED && bleComposite.isConnected())
  {
    ...
    else if (btn == keyboard_param.stop)
    {
      ...
      keyboard->mediaKeyPress(KEY_MEDIA_STOP);
      keyboard->mediaKeyRelease(KEY_MEDIA_STOP);
    ...
    {
      keyboard->mediaKeyPress(KEY_MEDIA_WWWHOME);
      keyboard->mediaKeyRelease(KEY_MEDIA_WWWHOME);
      lv_label_set_text_fmt(label1, "Media");
    }
  }
  else
  {
    lv_label_set_text_fmt(label1, "#ff0000 Not connected#");
